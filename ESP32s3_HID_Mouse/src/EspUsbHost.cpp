...
void EspUsbHost::begin(void)
{
  ...
  xTaskCreate(usbPollingTask, "usbPollingTask", USB_TASK_STACK_SIZE, this, USB_TASK_PRIORITY, &usbTaskHandle);
  xTaskCreate(clientEventTask, "clientEventTask", USB_TASK_STACK_SIZE, this, CLIENT_TASK_PRIORITY, &clientTaskHandle);
}

static void usbPollingTask(void *param)
{
  EspUsbHost *usbHost = static_cast<EspUsbHost *>(param);
  while (true)
  {
    esp_err_t err = usb_host_lib_handle_events(1, &usbHost->eventFlags);
    if (err != ESP_OK && err != ESP_ERR_TIMEOUT)
    {
      ESP_LOGI("EspUsbHost", "Handle Events Error: %x, EventFlags=%x", err, usbHost->eventFlags);
    }
  }
}

static void clientEventTask(void *param)
{
  EspUsbHost *usbHost = static_cast<EspUsbHost *>(param);
  while (true)
  {
    ...
    esp_err_t err = usb_host_client_handle_events(usbHost->clientHandle, 0);
    if (err != ESP_OK && err != ESP_ERR_TIMEOUT)
    {
      ESP_LOGI("EspUsbHost", "Client Handle Events Error: %x", err);
    }
    ...
    if (usbHost->isReady)
    {
      for (int i = 0; i < usbHost->usbTransferSize; i++)
      {
        ...
        // Only submit transfer for endpoint 81 Mouse
        if (usbHost->usbTransfer[i]->bEndpointAddress == 0x81 &&
            (usbHost->usbTransfer[i]->status == USB_TRANSFER_STATUS_COMPLETED ||
             usbHost->usbTransfer[i]->status == USB_TRANSFER_STATUS_CANCELED ||
             usbHost->usbTransfer[i]->status == USB_TRANSFER_STATUS_STALL))
        {
          ESP_LOGI("EspUsbHost", "Submitting Transfer: Endpoint=%x at time=%lld us", usbHost->usbTransfer[i]->bEndpointAddress, currentTime);
          err = usb_host_transfer_submit(usbHost->usbTransfer[i]);
          if (err != ESP_OK && err != ESP_ERR_NOT_FINISHED && err != ESP_ERR_INVALID_STATE)
          {
            ESP_LOGI("EspUsbHost", "Transfer Submit Error: %x", err);
          }
        }
      }
    }
    vTaskDelay(pdMS_TO_TICKS(2)); // HID Host limit :(
  }
}
...
void EspUsbHost::_onReceive(usb_transfer_t *transfer)
{
  ...
  // Only process endpoint 81 (mouse)
  if (transfer->bEndpointAddress != 0x81)
  {
    return;
  }
  ...
  {
    ESP_LOGI("EspUsbHost", "HID Interface: SubClass=%x, Protocol=%x", endpoint_data->bInterfaceSubClass, endpoint_data->bInterfaceProtocol);
    if (endpoint_data->bInterfaceSubClass == HID_SUBCLASS_BOOT && endpoint_data->bInterfaceProtocol == HID_ITF_PROTOCOL_MOUSE)
    {
      static uint8_t last_buttons = 0;
      ESP_LOGI("EspUsbHost", "Mouse Data Received");
      ...
      {
        ESP_LOGI("EspUsbHost", "Mouse Buttons Changed: %x", report.buttons);
        usbHost->onMouseButtons(report, last_buttons);
        last_buttons = report.buttons;
      }
      if (report.x != 0 || report.y != 0 || report.wheel != 0)
      {
        ESP_LOGI("EspUsbHost", "Mouse Moved: x=%d, y=%d, wheel=%d", report.x, report.y, report.wheel);
...
void EspUsbHost::onMouseMove(hid_mouse_report_t report)
{
  if (report.wheel != 0)
  {
    Serial1.printf("km.wheel(%d)\n", report.wheel);
  }
  else
  {
    Serial1.printf("km.move(%d,%d)\n", report.x, report.y);
  }
}
