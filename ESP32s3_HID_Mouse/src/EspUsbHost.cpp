...
static void usb_lib_task(void *arg)
{
  ...
  while (true)
  {
    ...
    esp_err_t err = usb_host_lib_handle_events(portMAX_DELAY, &event_flags); // Poll for USB Host Library events
    ...
    if (instance->clientHandle == NULL || !instance->isClientRegistering)
    {
      ...
      else
      {
        ...
        instance->isClientRegistering = true; // Set the flag only after successful registration
      }
    }
  }
}
...
void EspUsbHost::begin(void)
{
  ...
  ESP_LOGI("EspUsbHost", "usb_host_install() status: %d", err);
  if (err != ESP_OK)
  {
    return;
  }
  ...
  // Create tasks for USB host library and client handling
...
static void usb_client_task(void *arg)
{
  ...
  while (true)
  {
    // Skip the iteration if the client is not yet registered
    if (!instance->isClientRegistering)
    {
      vTaskDelay(pdMS_TO_TICKS(10)); // Allow device to connect
      continue;
    }

    ESP_LOGI("EspUsbHost", "Waiting for USB client events...");

    // Blocking call to wait for USB events
    esp_err_t err = usb_host_client_handle_events(instance->clientHandle, portMAX_DELAY);

    ESP_LOGI("EspUsbHost", "usb_host_client_handle_events() returned, err=%x", err);
    ...
    if (err == ESP_OK)
    {
      ...
      ESP_LOGI("EspUsbHost", "Event handled successfully.");
    }
    else if (err == ESP_ERR_TIMEOUT)
    {

      ESP_LOGI("EspUsbHost", "Timeout occurred, no events.");
    }
    else
    {

      ESP_LOGI("EspUsbHost", "Unexpected error or condition.");
    }

    if (instance->isReady)
    {
      for (int i = 0; i < instance->usbTransferSize; i++)
      {
        if (instance->usbTransfer[i] == NULL)
        {

          ESP_LOGI("EspUsbHost", "usbTransfer[%d] is NULL", i);
          continue;
        }

        if (instance->usbTransfer[i]->bEndpointAddress == 0x81)
        {

          err = usb_host_transfer_submit(instance->usbTransfer[i]);
        }
    ...
    //Serial0.println("lkj klj l;k jfwqkl;fj walf;dkj afl");
    //vTaskDelay(pdMS_TO_TICKS(2)); // Delay between iterations
  }
}
*/
...
void EspUsbHost::onConfig(const uint8_t bDescriptorType, const uint8_t *p)
{
  ...
  case USB_ENDPOINT_DESC:
  {
    ...
    if (ep_desc->bEndpointAddress & USB_B_ENDPOINT_ADDRESS_EP_DIR_MASK)
    {
      ...
      if (ep_desc->bEndpointAddress == 0x81)
      {
...
void EspUsbHost::_onReceive(usb_transfer_t *transfer)
{
  ...
  // Process data for endpoint 0x81
  if (transfer->bEndpointAddress == 0x81)
  {
    ...
    if (usbHost->endpoint_data_list[endpoint_num].bInterfaceClass == USB_CLASS_HID)
    {
      ...
      if (usbHost->endpoint_data_list[endpoint_num].bInterfaceSubClass == HID_SUBCLASS_BOOT && usbHost->endpoint_data_list[endpoint_num].bInterfaceProtocol == HID_ITF_PROTOCOL_MOUSE)
      {
        static uint8_t last_buttons = 0;

        hid_mouse_report_t report = {};
        report.buttons = transfer->data_buffer[0];
        report.x = transfer->data_buffer[2];
        report.y = transfer->data_buffer[4];
        report.wheel = transfer->data_buffer[6];
    ...
    // Re-submit the transfer for endpoint 0x81 (mouse)
