...
void EspUsbHost::monitor_inactivity_task(void *arg)
{
  ...
  while (true)
  {
    vTaskDelay(pdMS_TO_TICKS(100)); // Non-blocking delay for 100ms

    // Check for inactivity and if the device is not already suspended
    if (!usbHost->deviceSuspended && millis() - usbHost->last_activity_time > 10000) // 10 seconds of inactivity
    {
...
void EspUsbHost::get_device_status()
{
  ...
  transfer->data_buffer[1] = USB_B_REQUEST_GET_STATUS; // GET_STATUS request
...
void EspUsbHost::suspend_device()
{
  if (deviceSuspended)
  {
    // Serial0.println("Device is already suspended."); // We are sleeping
    return;
  }
  ...
  transfer->data_buffer[1] = USB_B_REQUEST_SET_FEATURE;          // SET_FEATURE request
  transfer->data_buffer[2] = USB_FEATURE_SELECTOR_REMOTE_WAKEUP; // Feature selector (low byte)
  transfer->data_buffer[3] = 0x00;
  transfer->data_buffer[4] = 0x00;
  transfer->data_buffer[5] = 0x00;
...
void EspUsbHost::resume_device()
{
  if (!deviceSuspended)
  {
    // Serial0.println("Device is not suspended.");
    return;
  }
  ...
  transfer->num_bytes = 8;
  transfer->data_buffer[0] = USB_BM_REQUEST_TYPE_DIR_OUT | USB_BM_REQUEST_TYPE_TYPE_STANDARD | USB_BM_REQUEST_TYPE_RECIP_DEVICE;
  transfer->data_buffer[1] = USB_B_REQUEST_CLEAR_FEATURE;        // CLEAR_FEATURE request
  transfer->data_buffer[2] = USB_FEATURE_SELECTOR_REMOTE_WAKEUP; // Feature selector (low byte)
  transfer->data_buffer[3] = 0x00;
  transfer->data_buffer[4] = 0x00;
  transfer->data_buffer[5] = 0x00;
...
void EspUsbHost::_onReceive(usb_transfer_t *transfer)
{
  ...
  ESP_LOGI("EspUsbHost", "URB Interrupt: Endpoint=%x, Bytes=%d", transfer->bEndpointAddress, transfer->actual_num_bytes);
  ...
  {
    ESP_LOGI("EspUsbHost", "HID Interface: SubClass=%x, Protocol=%x", usbHost->endpoint_data_list[endpoint_num].bInterfaceSubClass, usbHost->endpoint_data_list[endpoint_num].bInterfaceProtocol);
        ...
        usbHost->endpoint_data_list[endpoint_num].bInterfaceProtocol == HID_ITF_PROTOCOL_MOUSE)
    {
      ...
      // Print the data buffer values for debugging
      ESP_LOGI("EspUsbHost", "Data buffer values:");
      for (int i = 0; i < transfer->actual_num_bytes; i++)
      {
        ESP_LOGI("EspUsbHost", "transfer->data_buffer[%d] = %d", i, transfer->data_buffer[i]);
      }

      // Calculate the total length of the data to read based on HIDReportDesc sizes
      uint8_t data_length = HIDReportDesc.buttonSize + HIDReportDesc.xAxisSize + HIDReportDesc.yAxisSize + HIDReportDesc.wheelSize;

      if (transfer->actual_num_bytes >= data_length)
      {
        // Define offsets based on HIDReportDesc sizes
        uint8_t buttonOffset = 0;
        uint8_t xOffset = buttonOffset + HIDReportDesc.buttonSize;
        uint8_t yOffset = xOffset + HIDReportDesc.xAxisSize;
        uint8_t wheelOffset = yOffset + HIDReportDesc.yAxisSize;

        // Process buttons
        report.buttons = transfer->data_buffer[buttonOffset];
        ESP_LOGI("EspUsbHost", "Aligned button = %d", report.buttons);

        // Process X-axis
        report.x = transfer->data_buffer[xOffset];
        ESP_LOGI("EspUsbHost", "Aligned X = %d", report.x);

        // Process Y-axis
        report.y = transfer->data_buffer[yOffset];
        ESP_LOGI("EspUsbHost", "Aligned Y = %d", report.y);

        // Process Wheel
        report.wheel = transfer->data_buffer[wheelOffset];
        ESP_LOGI("EspUsbHost", "Aligned Wheel = %d", report.wheel);

        // Process Pan (assuming it is right after the wheel)
        uint8_t panOffset = wheelOffset + HIDReportDesc.wheelSize;
        report.pan = transfer->data_buffer[panOffset];
        ESP_LOGI("EspUsbHost", "Aligned Pan = %d", report.pan);
      }
      else
      {
        ESP_LOGE("EspUsbHost", "Error: Data buffer size is insufficient. Expected size: %d, Actual size: %d", data_length, transfer->actual_num_bytes);
      }
      ...
      if (report.x != 0 || report.y != 0 || report.wheel != 0 || report.pan != 0)
      {
        usbHost->onMouseMove(report);
      }
    }

    ESP_LOGI("EspUsbHost", "Transfer Completed: Endpoint=%x, State=%x", transfer->bEndpointAddress, transfer->status);

    if (transfer->status == USB_TRANSFER_STATUS_COMPLETED)
    {
      ESP_LOGI("EspUsbHost", "Transfer Completed Successfully");
    }
    else if (transfer->status == USB_TRANSFER_STATUS_STALL)
    {
      ESP_LOGI("EspUsbHost", "Transfer STALL Received");
    }
    else
    {
      ESP_LOGI("EspUsbHost", "Transfer Error or Incomplete: Status=%x", transfer->status);
    }

    // Re-submit the transfer for endpoint 0x81 (mouse) only if not suspended
    if (!usbHost->deviceSuspended)
    {
...
void EspUsbHost::_onReceiveControl(usb_transfer_t *transfer)
{
  ...
  for (int i = 8; i < transfer->actual_num_bytes; i++)
  {
    ...
    if ((i - 7) % 16 == 0) // Format the output with 16 bytes per line
    {
      printf("\n");
    }
  }
  ...
  // Handle the parsed descriptor
  handleHIDReportDescriptor(descriptor);

  usb_host_transfer_free(transfer);
}

// Helper function to get item size
...
EspUsbHost::HIDReportDescriptor EspUsbHost::parseHIDReportDescriptor(uint8_t *data, int length)
{
  ...
  HIDReportDescriptor HIDReportDesc;
  ...
  while (i < length)
  {
    ...
    bool isSigned = (item == 0x14 || item == 0x24); // LOGICAL_MINIMUM/MAXIMUM are signed
    ...
    case 0x04: // USAGE_PAGE
      ...
      printf("* Parsed USAGE_PAGE: %02X\n", usagePage); // Updated to print as 8-bit
      break;
    case 0x08: // USAGE
      usage = (uint8_t)value;
      printf("* Parsed USAGE: %02X\n", usage); // Updated to print as 8-bit
      break;
    case 0x84: // REPORT_ID
      reportId = value;
    ...
    case 0x24: // LOGICAL_MAXIMUM
      ...
      {
        logicalMax16 = value;
        printf("* Parsed LOGICAL_MAXIMUM (16-bit): %02X %02X\n", (uint8_t)(logicalMax16 & 0xFF), (uint8_t)((logicalMax16 >> 8) & 0xFF));
      }
      break;
    ...
    case 0x80: // INPUT
      ...
      {                                                                                     // X-axis or Y-axis
        HIDReportDesc.xAxisSize = HIDReportDesc.yAxisSize = (logicalMax16 <= 2047) ? 1 : 2; // hot fix int8 mouse
      }
      else if (usagePage == 0x01 && usage == 0x38)
      { // Wheel
        HIDReportDesc.wheelSize = (logicalMax16 <= 127 && logicalMax16 >= -128) ? 1 : 2;
      }
      else if (usagePage == 0x09 && usage >= 0x01 && usage <= 0x10)
      { // Buttons
        printf("* Parsed BUTTONS: Report Count: %d\n", reportCount);
        HIDReportDesc.buttonSize = (reportCount <= 8) ? 1 : 2;
        printf("* Calculated Button Bitmap Size: %d\n", HIDReportDesc.buttonSize);
      }
  ...
  // Handle the parsed HID report descriptor
  handleHIDReportDescriptor(HIDReportDesc);
