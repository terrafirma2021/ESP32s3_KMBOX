...
{
    // Initialize SPIFFS
    if (!SPIFFS.begin(true)) // Passing true to format the filesystem if mounting fails
    {
    ...
    // Serial0.println("Preparing to write config data"); <------ Dun Debug
...
// Function to print the contents of the configuration file
void printConfigFile()
{
    File configFile = SPIFFS.open(configFilePath, FILE_READ);
    if (!configFile)
