...
void setupDMA() {
    // Set the onReceive callback function for UART receive events
    Serial1.onReceive([](){
...
void IRAM_ATTR onDMAReceive1(void *arg) {
    while (Serial1.available()) {
        ...
        if (nextHead != dmaBufferTail1) { // Check for buffer overflow
...
void IRAM_ATTR onDMAReceive0(void *arg) {
    while (Serial0.available()) {
        ...
        if (nextHead != dmaBufferTail0) { // Check for buffer overflow
